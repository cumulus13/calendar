...
class Kalender:
    ...
    def display_current_datetime(self):
        """Display current date and time in a formatted panel at the bottom"""
        # Format the datetime
        day_name = self.current_datetime.strftime("%A")
        date_str = self.current_datetime.strftime("%d %B %Y")
        time_str = self.current_datetime.strftime("%H:%M:%S")
        
        # Create formatted text
        datetime_text = Text()
        datetime_text.append("Current Date & Time\n", style="bold #FF5500 underline")
        datetime_text.append(f"{day_name}, ", style="bold #55FFFF")
        datetime_text.append(f"{date_str}\n", style="bold #FFFF00")
        datetime_text.append(f"{time_str}", style="bold #00FF00")
        
        # Create panel with the datetime
        panel = Panel(
            datetime_text,
            border_style="#FF5500",
            padding=(1, 2),
    ...
    def display_full_year_in_grid(self, year):
        ...
        # Print the entire calendar in grid format (4 months per row)
        terminal_width = shutil.get_terminal_size()[0]
        for i in range(0, 12, 4):  # Step through months 4 at a time
            row = Columns(month_blocks[i:i+4], expand=True)
            console.print(row)
            # Add separator line after each row except the last one
            if i < 8:  # Don't add line after the last row
                console.print("─" * terminal_width, style="#555555")
        
        # Display current datetime at the bottom
